...
import com.example.corpout.param.BaseReqParam;
...
public class YQUtil {
    ...
    public static String sendService(BaseReqParam baseReqParam) {
        StringBuilder rcvMsg = new StringBuilder();
        String yqdm = baseReqParam.getYqdm();//银企代码
        String tranCode = baseReqParam.getTranCode();//交易码
        boolean chkAutoHead = baseReqParam.isChkAutoHead();
        boolean chkAutoSubfixLenth = baseReqParam.isChkAutoSubfixLenth();
        boolean chk0x00 = baseReqParam.isChk0x00();
        String txtSend = baseReqParam.getTxtSend();
        String packets = "";
        if (chkAutoHead) {
            Assert.hasText(yqdm, "银企代码不能为空");
            Assert.hasText(tranCode, "交易码不能为空");
            String txtYqdm = yqdm;
        ...
        try {
            Packets packetsRP = YQUtil.send2server("122.51.35.76", 7072, packets, PROTOCAL_TCP);
